...
if command -v python3 >/dev/null 2>&1; then
  ...
  echo "Error: python3 or python is required to run Concord gene fine-tuning." >&2
  exit 1
fi
...
CONFIG_PATH="${FINETUNE_CONFIG:-${ROOT_DIR}/configs/finetune_gene.yaml}"
EXTRA_ARGS=()
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --config)
      ...
      ;;
    -h|--help)
      exec "${PYTHON_BIN}" -u -m src.train.finetune --help
      ;;
    *)
      EXTRA_ARGS+=("$1")
      shift
...
if [[ ! -f "${CONFIG_PATH}" ]]; then
  echo "Error: gene fine-tune config file not found: ${CONFIG_PATH}" >&2
  exit 1
fi
...
if [[ "${NPROC_PER_NODE}" -gt 1 ]]; then
  exec torchrun --nproc_per_node="${NPROC_PER_NODE}" -m src.train.finetune "${ARGS[@]}"
fi

exec "${PYTHON_BIN}" -u -m src.train.finetune "${ARGS[@]}"
